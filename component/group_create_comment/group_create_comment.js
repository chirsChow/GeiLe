var group_create_comment_myConfig = function($stateProvider){
    $stateProvider
    .state('tab.group_create_comment', {
        url: '/group_create_comment/:merchantNo/:industry/:hash/:groupId',
        hideTab:true,
        views: {
            'tab-mine': {
                templateUrl: 'component/group_create_comment/group_create_comment.html',
                controller: 'group_create_commentCtrl'
            }
        }
    });
};
myapp.config(group_create_comment_myConfig);

angular.module('starter.group_create_comment',[])
.controller('group_create_commentCtrl', function($scope,$state,$document,Common,$http,$stateParams,toast,$q,$rootScope,$document,$timeout) {
    ...
    $scope.createComment=function () {
        var data = {labels:[]}
        data.paymentNo=$scope.chooseItems.paymentNo
